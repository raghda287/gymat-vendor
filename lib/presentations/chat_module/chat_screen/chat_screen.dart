...
class _ChatScreenState extends State<ChatScreen>
  ...
  final ChatProvider provider = getIt();
  final ProfileProvider profileProvider = getIt();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      await provider.createChatRoom(
        ...
        widget.chatUser.room_id,
      );

      provider.selectedRoom = provider.roomModel;

      final roomId = provider.roomModel?.id ?? widget.chatUser.room_id;

      if (roomId != null) {
        await provider.refreshCurrentRoomMessages(
          widget.chatUser.id!,
          widget.chatUser.name,
          widget.chatUser.image,
          roomId,
      ...
      preferences.saveChatNotificationData(
        ChatNotificationHandler(roomId, false),
      );

      _startMessagesRefresh();
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions:
        profileProvider.getUserModel()?.providerModel?.mainAccount?.category.type ==
            USERTYPE.coache.name
            ? [
          IconButton(
            onPressed: () {
              provider.createChatVideoUrl(
                widget.chatUser.room_id,
                widget.chatUser.id,
              );
            },
            icon: const CustomSvgIcon(
              assetName: 'create_video',
              width: 26,
              height: 26,
              color: mainColor,
            ),
          ...
          IconButton(
            onPressed: () {
              NavigatorHandler.push(
                UserProfileScreen(userId: widget.chatUser.id),
              );
            },
            icon: const CustomSvgIcon(
              assetName: 'info',
      ...
      body: Column(
        children: [
                ...
                children: [
                  Opacity(
                    ...
                    child: CustomAssetImage(
                      assetName:
                      AppTheme.isDarkMode() ? 'chat_bg_dark' : 'chat_bg_white',
                      width: 512,
                      height: 1024,
                    ),
                  ),
          ...
          profileProvider.getUserModel()?.providerModel?.mainAccount?.is_message ==
              true
              ? MessageContanerWidget(
            onPressed: () {
              if (_controller.status == AnimationStatus.forward ||
                  _controller.status == AnimationStatus.completed) {
                _controller.reverse();
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: CustomText(
                    title: 'Cannot send message at the moment'.tr(),
                    fontSize: 12,
                    fontColor:
