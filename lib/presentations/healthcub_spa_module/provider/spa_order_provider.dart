...
class SpaOrderProvider with ChangeNotifier {
  final SpaOrderRepository repository = getIt();
  bool isLoading = false;
  bool isLoadingOrderDetails = false;

  List<GeneralOrderModel> orders = [];
  SpaOrderDetailsModel? spaOrderDetailsModel;
  ...
  void getOrderDetails(num orderId) async {
    ...
    notifyListeners();
    try {
      ApiResponse response = await repository.getSpaOrderDetails(orderId);
      isLoadingOrderDetails = false;
        ...
        if (response.response!.statusCode == 200 || response.response!.statusCode == 201) {
          if (response.response!.data != null &&response.response!.data['code']==200) {
            if ( response.response!.data['data'] != null) {
              spaOrderDetailsModel = SpaOrderDetailsModel.fromJson(response.response!.data['data']);
    ...
    try {
      ...
      if (response.response != null) {
        if (response.response!.statusCode == 200 || response.response!.statusCode == 201) {
          if (response.response!.data != null &&response.response!.data['code']==200) {
            if ( response.response!.data['data'] != null) {
              spaOrderDetailsModel!.status = status;
