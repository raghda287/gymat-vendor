...
class ShopHomeProvider with ChangeNotifier{
  ...
  void getDepartments() async{
    ...
    notifyListeners();
    try {
      ApiResponse response = await repository.getCategories(6,null);
  ...
  Future<ApiResponse> scanQrCode(num? orderId) async {
    ...
    await dialog.show();
    ShopHomeRepository shopHomeRepository = getIt();
    ApiResponse response = await shopHomeRepository.scanQrCode(orderId);
    await dialog.hide();
    return response;
  }

  void getStatistics() async {
    ShopHomeRepository repository = getIt();
