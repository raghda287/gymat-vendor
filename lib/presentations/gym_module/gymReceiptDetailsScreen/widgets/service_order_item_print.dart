...
class ServiceOrderItemPrint extends StatelessWidget {
  final GymOrderDetailsModel model ;
  const ServiceOrderItemPrint({super.key, required this.model});
          ...
          children: [
            ...
            Expanded(child: Padding(
              ...
              child: Column(
                ...
                children: [
                  CustomText(title: model.order?.service_option!=null?'${model.order?.service?.getTitle()} - ${model.order?.service_option?.getTitle()}':'${model.order?.service?.getTitle()}',fontColor: Colors.black,fontSize: 14,maxLines: 1,),
                  const SizedBox(height: 8,),
                  Row(
                    children: [
                      CustomText(title: 'Price:'.tr(),fontColor: greyColor,fontSize: 12,maxLines: 1,),
                      const SizedBox(width: 4,),
                      CustomText(title: '${CustomNumberFormat.format(model.grand_total??0)} ${'SAR'.tr()}',fontColor:Colors.black,fontSize: 13,maxLines: 1,fontWeight: FontWeight.bold,),
                      const SizedBox(width: 4,),
