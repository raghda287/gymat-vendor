...
import 'package:gymatvendor/presentations/widgets/add_discount_widget/addDiscountWidget.dart';
...
import 'package:gymatvendor/presentations/widgets/drop_down_widget/drop_down_membership_widget.dart';
...
class GymAddService extends StatefulWidget {
  final ServiceModel? serviceModel;
  const GymAddService({super.key, this.serviceModel});

  @override
  State<GymAddService> createState() => _GymAddServiceState();
}

class _GymAddServiceState extends State<GymAddService> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) { gymServicesProvider.initAddService();
    if(widget.serviceModel!=null){
      gymServicesProvider.servicePhotoPath = widget.serviceModel!.photo;
               ...
               Visibility(
                 ...
                 child: Column(
                   ...
                   children: [
                     ...
                     DropDownMembershipDepartmentWidget(departments: provider.departments,defaultValue: widget.serviceModel?.category, onValueSelected: (value) {
                       if(value.id==1){
                         NavigatorHandler.pushReplacement(const GymAddMemberShipService());
                       }else{
                ...
                const SizedBox(height: 12,),
                CustomTextFormField(controller: _servicePriceController,textInputType: const TextInputType.numberWithOptions(signed: false,decimal: true),maxLength: 6,minPrefixSuffixWidth: 32,suffix:CustomSvgIcon(assetName: 'sar',width: 14,height: 14,color: AppTheme.isDarkMode()?Colors.white:Colors.black,),),
                ...
                Row(
                  children: [
                    Expanded(child: CustomTextFormField(controller: _serviceTimeController,textInputType: const TextInputType.numberWithOptions(signed: false,decimal: false),maxLength: 2,)),
                    const SizedBox(width: 16,),
                    DropDownDays(list: minuteUnitType,defalutValue: widget.serviceModel!=null?getMinuteUnitType(widget.serviceModel!.service_time_name!):null, onValueSelected: (value) {
                ...
                CustomButton(title: 'Confirm'.tr(), onTap: (){
                  ...
                  if(departmentModel!=null&&provider.servicePhotoPath!=null&&name.isNotEmpty&&regExp.hasMatch(price)&&time.isNotEmpty&&!time.startsWith('0')&&description.isNotEmpty){
                    ...
                    if(widget.serviceModel==null){
                      provider.addService(departmentModel!.id!, name, price, time, durationUnitType!, description,discount,isDiscountActive,discountType);

                    }else{
                      provider.updateService(widget.serviceModel!.id!,departmentModel!.id!, name, price, time, durationUnitType!, description,discount,isDiscountActive,discountType);
