...
import 'package:gymatvendor/presentations/gym_module/home_screen/widgets/card_order_placeholder_widget.dart';
import 'package:gymatvendor/presentations/gym_module/home_screen/widgets/card_order_widget.dart';
import 'package:gymatvendor/presentations/gym_module/home_screen/widgets/card_scan_qr_widget.dart';
import 'package:gymatvendor/presentations/gym_module/home_screen/widgets/unAccepted_widget.dart';
...
import '../gym_services_screen/gym_services_screen.dart';

class GymHomeScreen extends StatefulWidget {
  final LocalNotificationHandler? localNotificationHandler;
  const GymHomeScreen({super.key, this.localNotificationHandler});

  @override
  State<GymHomeScreen> createState() => _GymHomeScreenState();
}

class _GymHomeScreenState extends State<GymHomeScreen> with WidgetsBindingObserver{
              ...
              child: Column(
                ...
                children: [
                  ...
                  ListTile(
                    ...
                    subtitle: CustomText(
                      title: 'Manage your booking'.tr(),
                      fontColor: greyColor,
                      fontSize: 12,
                    ),
                  ),
                  Consumer<GymHomeProvider>(builder: (context, provider, _) {
                    return Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 12.0),
                      child: Row(
                        children: [
                          provider.isLoadingStatistics ||
                                  provider.statisticsHomeModel == null
                              ? const CardOrderPlaceholderWidget()
                              : CardOrderWidget(
                                  onTap: () {
                                    ProfileProvider provider = getIt();
                                    ...
                                    if(provider.getUserModel() != null &&!provider.getUserModel()!.providerModel!.is_accepted){
                                      ...
                                      NavigatorHandler.push(const GymOrderScreen(
                                          orderType: 'up_coming'));
                                    }

                                  },
                                  orderCount: provider.statisticsHomeModel
                                              ?.booking?.newCount !=
                                          null
                                      ? '${provider.statisticsHomeModel?.booking?.newCount}'
                                      : '0',
                          ...
                          provider.isLoadingStatistics ||
                              ...
                              : CardOrderWidget(
                                  ...
                                  orderCount: provider.statisticsHomeModel
                                              ?.booking?.end !=
                                          null
                                      ? '${provider.statisticsHomeModel?.booking?.end}'
                                      : '0',
                                  title: 'Completed'.tr(),
                                ),
                          const SizedBox(
                  ...
                  Consumer<GymHomeProvider>(builder: (context, provider, _) {
                    return provider.isLoadingStatistics ||
                            provider.statisticsHomeModel == null
                        ? const CardChartPlaceholderWidget()
                        : CardChartWidget(
                            earningPrice:
                                '${provider.statisticsHomeModel?.goals?.total ?? 0}',
                  ...
                  ListTile(
                    horizontalTitleGap: 4,
                    leading: CustomSvgIcon(
                      assetName: 'add_service',
                      width: 16,
                      height: 16,
                      color: AppTheme.isDarkMode() ? Colors.white : mainColor,
                    ),
                    title: CustomText(
                      title: 'Add your Services'.tr(),
                      fontWeight: FontWeight.bold,
                      fontColor:
                          AppTheme.isDarkMode() ? Colors.white : mainColor,
                      fontSize: 14,
                    ...
                    subtitle: CustomText(
                      title: 'Add your services, Edit, delete...'.tr(),
                      fontColor: greyColor,
                      fontSize: 12,
                    ),
                    trailing: InkWell(
                        ...
                        onTap: () {
                          ...
                          if(provider.getUserModel() != null &&!provider.getUserModel()!.providerModel!.is_accepted){
                            ...
                            NavigatorHandler.push(const GymServicesScreen());
                          ...
                          : GridView.builder(
                              ...
                              itemBuilder: (context, index) {
                                return HomeCategoryWidget(
                                    title: provider
                                            .departments[index].trans_title!
                                            .contains('trans')
                                        ? provider.departments[index].title ??
                                            ''
                                        : provider.departments[index]
                                                .trans_title ??
                                            '',
                                    onTap: () {
                                      NavigatorHandler.push(GymServicesScreen(
                                        homeDepartmentModel:
                                            provider.departments[index],
                                      ));
                                    });
