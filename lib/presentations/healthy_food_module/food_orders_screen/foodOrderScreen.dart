...
import '../healthyFoodOrderDetails/foodOrderDetailsScreen.dart';
import '../provider/food_order_provider.dart';

class FoodOrderScreen extends StatefulWidget {
  final String orderType;
  const FoodOrderScreen({super.key,required this.orderType});
...
class _FoodOrderScreenState extends State<FoodOrderScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<FoodOrderProvider>(builder: (context,provider,_){
      ...
      ListView.builder(
          ...
          itemBuilder: (context,index){
            ...
            return InkWell(
                ...
                child: FoodOrderItem(model: model,index: index,));
  ...
  void _onTap(GeneralOrderModel model){
    if(model.market!=null&&model.market!.category.type==USERTYPE.healthyFood.name){
      NavigatorHandler.push(FoodOrderDetailsScreen(orderId: model.id!));
    }
  }
}
