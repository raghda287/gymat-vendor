...
import 'package:gymatvendor/data/repositories/food_home_repository.dart';
...
class HealthyFoodHomeProvider with ChangeNotifier{
  ...
  void getDepartments() async{
    ...
    try {
      ...
      print('health food category error>>>${e.toString()}');
  ...
  Future<ApiResponse> scanQrCode(num? orderId) async {
    ...
    await dialog.show();
    FoodHomeRepository foodHomeRepository = getIt();
    ApiResponse response = await foodHomeRepository.scanQrCode(orderId);
    await dialog.hide();
    return response;
  }

  void getStatistics() async {
    FoodHomeRepository repository = getIt();
    ...
    try {
      ...
      print('food statistics error>>>${e.toString()}');
    }
  }
