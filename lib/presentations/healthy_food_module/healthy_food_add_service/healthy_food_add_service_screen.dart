...
import 'package:gymatvendor/presentations/healthcub_spa_module/provider/spa_services_provider.dart';
...
import '../provider/healthy_food_services_provider.dart';

class HealthyFoodAddServiceScreen extends StatefulWidget {
  final ServiceModel? serviceModel;
  const HealthyFoodAddServiceScreen({super.key, this.serviceModel});

  @override
  State<HealthyFoodAddServiceScreen> createState() => _HealthyFoodAddServiceScreenState();
}

class _HealthyFoodAddServiceScreenState extends State<HealthyFoodAddServiceScreen> {
  HealthyFoodServicesProvider healthyFoodServicesProvider =getIt();
  DepartmentModel? departmentModel;
  final TextEditingController _serviceNameController = TextEditingController();
  final TextEditingController _servicePriceController = TextEditingController();
  final TextEditingController _servicePreperationTimeController = TextEditingController();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      healthyFoodServicesProvider.initAddService();
    if(widget.serviceModel!=null){
      healthyFoodServicesProvider.servicePhotoPath = widget.serviceModel!.photo;
      ...
      _servicePreperationTimeController.text = widget.serviceModel!.preparation_time!=null?widget.serviceModel!.preparation_time!.toString()??'':'';
      _caloriesController.text = widget.serviceModel!.calories!=null?widget.serviceModel!.calories!.toString()??'':'';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<HealthyFoodServicesProvider>(
            ...
            child: Column(
              ...
              children: [
               Visibility(
                ...
                Row(
                  ...
                  children: [
                    ...
                    InkWell(
                        ...
                        child: CustomRoundedImage(width: 64,height: 64*3/4,radius: 4,elevation: 0,url: provider.servicePhotoPath,))
                  ],
                ),
                const SizedBox(height: 12,),
                CustomText(title: 'Price'.tr(),fontColor: AppTheme.isDarkMode()?Colors.white:Colors.black,),
                const SizedBox(height: 12,),
                CustomTextFormField(controller: _servicePriceController,textInputType: const TextInputType.numberWithOptions(signed: false,decimal: true),maxLength: 6,minPrefixSuffixWidth: 32,
                ...
                CustomText(title: 'Preparation time'.tr(),fontColor: AppTheme.isDarkMode()?Colors.white:Colors.black,),
                const SizedBox(height: 12,),
                CustomTextFormField(controller: _servicePreperationTimeController,textInputType: const TextInputType.numberWithOptions(signed: false,decimal: false),maxLength: 4,minPrefixSuffixWidth: 48,suffix: CustomText(title: 'Mins'.tr(),fontColor: AppTheme.isDarkMode()?Colors.white:Colors.black,fontWeight: FontWeight.bold,fontSize: 17,)),
                const SizedBox(height: 12,),
                CustomText(title: 'Calories'.tr(),fontColor: AppTheme.isDarkMode()?Colors.white:Colors.black,),
                const SizedBox(height: 12,),
                CustomTextFormField(controller: _caloriesController,textInputType: const TextInputType.numberWithOptions(signed: false,decimal: false),maxLength: 4,minPrefixSuffixWidth: 48,suffix: CustomText(title: 'Cal'.tr(),fontColor: AppTheme.isDarkMode()?Colors.white:Colors.black,fontWeight: FontWeight.bold,fontSize: 17,)),
                ...
                CustomButton(title: 'Confirm'.tr(), onTap: (){
                  ...
                  String time = _servicePreperationTimeController.text.trim();
                  ...
                  if(departmentModel!=null&&provider.servicePhotoPath!=null&&name.isNotEmpty&&regExp.hasMatch(price)&&time.isNotEmpty&&time!='0'&&description.isNotEmpty&&calories.isNotEmpty){
                    ...
                    if(widget.serviceModel==null){
                      provider.addService(departmentModel!.id!, name, price, time,calories, description,discount,isDiscountActive,discountType);

                    }else{
                      provider.updateService(widget.serviceModel!.id!,departmentModel!.id!, name, price, time,calories, description,discount,isDiscountActive,discountType);
                    ...
                    CustomScaffoldMessanger.showToast(title: 'Preparation time field is required'.tr());

                  }else if(time=='0'){
                    CustomScaffoldMessanger.showToast(title: 'Preparation time is invalid'.tr());

                  }else if(calories.isEmpty){
                    CustomScaffoldMessanger.showToast(title: 'Calories field is required'.tr());
                ...
                children: [
                  InkWell(
                    onTap: () async{
                      ...
                      NavigatorHandler.pop();
                      healthyFoodServicesProvider.pickImage(ImageSource.camera,);
                    },
                    child: Column(
                      children: [
                        CustomSvgIcon(
                  ...
                  InkWell(
                    onTap: () {
                      NavigatorHandler.pop();
                      healthyFoodServicesProvider.pickImage(ImageSource.gallery);
                    },
                    child: Column(
                      children: [
                        CustomSvgIcon(
