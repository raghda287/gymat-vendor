...
import 'package:gymatvendor/data/repositories/sports_field_order_repository.dart';
...
class SportsFieldOrderProvider with ChangeNotifier {
  final SportsFieldOrderRepository repository = getIt();
  bool isLoading = false;
  bool isLoadingOrderDetails = false;

  List<GeneralOrderModel> orders = [];
  SportsFieldOrderDetailsModel? sportsFieldOrderDetailsModel;
  ...
  void initSportsFieldOrderDetails() {
    isLoadingOrderDetails = false;
    sportsFieldOrderDetailsModel = null;
  }
  ...
  void getOrderDetails(num orderId) async {
    ...
    notifyListeners();
    try {
      ApiResponse response = await repository.getSportsFieldOrderDetails(orderId);
      isLoadingOrderDetails = false;
        ...
        if (response.response!.statusCode == 200 || response.response!.statusCode == 201) {
          if (response.response!.data != null &&response.response!.data['code']==200) {
            if ( response.response!.data['data'] != null) {
              sportsFieldOrderDetailsModel = SportsFieldOrderDetailsModel.fromJson(response.response!.data['data']);
    ...
    try {
      ...
      if (response.response != null) {
        if (response.response!.statusCode == 200 || response.response!.statusCode == 201) {
          if (response.response!.data != null &&response.response!.data['code']==200) {
            if ( response.response!.data['data'] != null) {
              sportsFieldOrderDetailsModel!.status = status;

              if(status=='end'||status=='refused'){
                getOrders('up_coming');
              }else{
                //sportsFieldOrderDetailsModel!.order?.start_date = startDate;
              }
