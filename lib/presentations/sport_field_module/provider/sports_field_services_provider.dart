...
import 'package:gymatvendor/data/repositories/sports_field_repository.dart';
...
import 'sports_field_home_provider.dart';

class SportsFieldServicesProvider with ChangeNotifier {
  ItemScrollController? departmentItemScrollController;
  ...
  void initAddService() {
    ...
    departmentItemScrollController = ItemScrollController();
    getDepartments(null);
  }
          ...
          if (homeDepartmentModel != null) {
            for (int index = 0; index < departments.length; index++) {
              if (departments[index].id == homeDepartmentModel.id) {
                selectedCategoryIndex = index;
                departmentItemScrollController?.jumpTo(index: index);
                getServiceByDepartment(false);
                break;
              }
            }
  ...
  void addDepartment(String title) async {
    ...
    try {
      ...
      if (response.response != null && response.response!.statusCode == 200) {
        if (response.code == 200) {
          if (response.response!.data != null &&
            ...
            sportsFieldHomeProvider.addNewDepartment(departmentModel);
          }
        }
      } else {
        if (response.error != null) {
      ...
      print('sports field add category error>>>${e.toString()}');
    }
  }
  ...
  void updateDepartment(DepartmentModel model, String title, index) async {
    ...
    try {
      ...
      if (response.response != null && response.response!.statusCode == 200) {
        if (response.code == 200) {
          if (response.response!.data != null &&
            ...
            notifyListeners();
            SportsFieldHomeProvider sportsFieldHomeProvider = getIt();
            sportsFieldHomeProvider.updateDepartmentItem(departmentModel);
          }
        }
      } else {
        if (response.error != null) {
      ...
      print('sports field update category error>>>${e.toString()}');
    }
  }
  ...
  void deleteDepartment(DepartmentModel model) async {
    ...
    try {
      ...
      print('sports field delete category error>>>${e.toString()}');
    }
  }
  ...
  void getServiceByDepartment(bool isLoadMore) async {
    ...
    try {
      ...
      print('sports field services error>>>${e.toString()}');
    }
  }

  void addService(num departmentId, String serviceName, String servicePrice,
      String serviceTime, String description,String discount,bool isDiscountActive,String discountType) async {
    ProgressDialog dialog = createProgressDialog(
        context: navigatorKey.currentContext!, msg: 'Adding...'.tr());
    await dialog.show();
    try {
      ...
      print('sports field service add error>>>${e.toString()}');
    }
  }
  ...
  void updateService(
    ...
    try {
      ...
      print('sports field service update error>>>${e.toString()}');
    }
  }
  ...
  void deleteService(ServiceModel model) async {
    ...
    try {
      ...
      print('sports field delete service error>>>${e.toString()}');
    }
  }
}
